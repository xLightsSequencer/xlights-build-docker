...
RUN DEBIAN_FRONTEND=noninteractive TZ=Etc/UTC apt-get update && \
    DEBIAN_FRONTEND=noninteractive TZ=Etc/UTC apt-get install -y g++ gcc build-essential cmake python3 wget curl git fuse colormake cbp2make patchelf gpgv ccache
RUN DEBIAN_FRONTEND=noninteractive TZ=Etc/UTC apt-get update && \
    DEBIAN_FRONTEND=noninteractive TZ=Etc/UTC apt-get install -y libgtk-3-dev libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev freeglut3-dev libavcodec-dev libavformat-dev libswscale-dev libsdl2-dev libavutil-dev libavfilter-dev libportmidi-dev libzstd-dev libcurl4-openssl-dev libltc-dev librsvg2-dev liblua5.3-dev libwebp-dev libsecret-1-dev libegl-dev libfreetype-dev libharfbuzz-dev libfontconfig-dev glslc libavahi-compat-libdnssd-dev zlib1g-dev libswresample-dev libminizip-dev libxlsxwriter-dev glslang-dev libxrandr-dev
...
ARG WXWIDGETS_TAG=xlights_2026.17c
RUN cd / && \
